...
leaves. `reason` accepts the same argument, where it narrows what the reasoning
agent may look at.
...
client.retrieve(
    "support",
    "what did we decide about the checkout rewrite",
    tag_groups=[{"tags": ["name:typsecript", "name:chekcout"], "resolve": "fuzzy"}],
)
```
...
`resolve` may only be combined with a `match` of `any` or `any_strict`, or left
unset: a resolved leaf is a set of alternatives, so `all_strict` would ask one
memory to carry every alternative at once.
