...
export interface RequestOptions {
   ...
   * a slow `reason` must not be replayed into two or three overlapping runs at a
   ...
   * `reason` raises it to the API's own ~90s synthesis budget so a normal
   * multi-iteration synthesis is not cut off mid-flight — safe only because
...
// storage, so nothing was stored. A 5xx is only safe to retry when the call
